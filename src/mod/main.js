var Draggable = require("drag").create;
...
function updateLevelsList() {
  ...
  div.innerHTML = "";
  var btn;
  for (var k = 0 ; k < LEVELS.length / 5 ; k += 16) {
    btn = tag("button");
    btn.className = "level";
    btn.textContent = k / 16 + 1;
...
function updateLevelDisplay(level) {
  ...
  glasses.forEach(function(glass, idxGlass) {
    categories.forEach( function(category) {
      var id = glass + "-" + category;
      console.info("[main] id=...", id);
      $(id).textContent = level[category][idxGlass];
      $(glass + "-beer").style.top = ((1 - level.current[idxGlass] / level.capacity[idxGlass]) * 100) + "%";
    });
  });
}
  ...
  function(id) {
    ...
    interact("#" + id + "-drag")
      ...
      .dropzone({
        overlap: .2,
        ondrop: function (evt) {
          var srcId = evt.relatedTarget.getAttribute("id").substr(0, 1);
          var dstId = evt.target.getAttribute("id").substr(0, 1);
